...
"""v2.4.5 reviewer-facing queue deterministic/live smoke acceptance."""
import csv
import json
import subprocess
import sys
...
SCRIPT = HOME / ".hermes/scripts/generate-review-queue-v245.py"
...
def set_cohort():
    dep = "dep-v245-review-" + hex(int(time.time()))[2:]
    data = {
        "deployment_id": dep,
        "deployment_timestamp": now(),
        "plugin_version": "2.4.5",
        "plugin_artifact_hash": "v245-reviewer-queue-local",
        "schema_version": "1.2",
        "cohort_label": "v2.4.5_review_queue",
    }
...
def requester_for(kind, i):
    ...
    if kind == "cron":
        return {"actor_type": "scheduler", "actor_id": "cron:v245-smoke", "request_channel": "cron", "requester_peer_id": "", "processing_peer_id": "peer70"}
...
def emit_case(i, kind="hmp", target="peer128", traffic_type="acceptance_test", text=None):
    ...
    sid = "v245-smoke-sess-%02d" % i
    eid = "v245-smoke-ep-%02d" % i
    tid = "v245-smoke-turn-%02d" % i
    task = "v245-smoke-task-%02d" % i
    tool = "v245-smoke-tool-%02d" % i
    code_hash = "v245-smoke-code-%02d" % i
    ret_outer = event_store.emit_retrieval(
        ...
        provenance_detail="v2.4.5 reviewer queue smoke",
...
def main():
    ...
    # add one organic peer row to prove organic queue generation, separate from acceptance rows
    emitted.append(emit_case(99, kind="hmp", target="peer128", traffic_type="organic_peer", text="check HMP health for peer128"))
