    ...
    Returns:
        {
            ...
            "fritzos_version": str,
            "product": str,
            "uptime_since": int,   # epoch of IPv4 lease
            "error": str or None
        }
    """
    try:
...
def get_lan_devices():
    """Return list of LAN devices from FritzBox.

    Returns:
        [{"name": str, "ip": str, "mac": str, "online": bool, "interface": str}, ...]
    """
    try:
        data = _post_data_lua("net")
        ...
        for dev in devices:
            result.append({
                ...
                "ip": dev.get("ip", "-"),
                "mac": dev.get("mac", "-"),
                "online": dev.get("stateinfo", {}).get("online", False),
...
def get_upnp_status():
    """Return WAN status from upnpc CLI.

    Returns dict or None on error.
    """
    import subprocess
    try:
        r = subprocess.run(["upnpc", "-s"], capture_output=True, text=True, timeout=10)
        out = r.stdout
        status = {}
        for line in out.split("\n"):
            line = line.strip()
            if line.startswith("Status :"):
                status["wan_status"] = line.split(":")[1].strip().split(",")[0]
            if "uptime=" in line:
                import re
                m = re.search(r'uptime=(\d+)s', line)
                if m:
                    status["uptime_s"] = int(m.group(1))
            if "ExternalIPAddress" in line:
                status["external_ip"] = line.split("=")[1].strip()
            if "MaxBitRateDown" in line:
                m = re.search(r'MaxBitRateDown\s*:\s*(\d+)', line)
                if m:
                    status["max_down_bps"] = int(m.group(1))
            if "MaxBitRateUp" in line:
                m = re.search(r'MaxBitRateUp\s*(\d+)', line)
                if m:
                    status["max_up_bps"] = int(m.group(1))
        return status if status.get("wan_status") else None
    except Exception:
        return None
