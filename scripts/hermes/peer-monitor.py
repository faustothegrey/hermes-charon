#!/usr/bin/env python3
"""peer-monitor.py — Orchestratore rete peer (peer70)
Verifica lo stato dei peer via API Hermes (:8642) e scopre nuovi dispositivi LAN.
Eseguito ogni ora da cronjob Hermes.
Tutta la comunicazione tra peer avviene via API, NON via SSH.
"""
import subprocess
import json
import os
...
KNOWN_FILE = HOME / ".hermes/peer-network/known_peers.txt"
...
def discover_arp():
    ...
    for line in out.splitlines():
        ...
        if state == "FAILED":
            continue
        # Non mostrare i peer già noti
        if any(ip == p[1] or ip == p[1] for p in KNOWN_PEERS):
            continue
        # Risolvi hostname
        hostname = ip
...
def main():
    ...
    lines.append("")
    lines.append("## Metriche Sistema (peer70 — orchestratore)")
    ...
    # === SAVE STATUS FILE ===
    md_lines = [
        "# 🌐 Peer Network — Stato Orchestrato da peer70",
        "",
    ...
    json_data = {
        "timestamp": now,
        "epoch": epoch,
        "orchestrator": "peer70 (192.168.178.70 - Raspberry Pi)",
        "peers": {},
        "system": {
