#!/usr/bin/env python3
"""Undervoltage watchdog — avvisa su Telegram quando il Pi è in undervoltage.

Pattern watchdog: stdout vuoto = silenzio; stdout non vuoto = consegnato
verbatim al canale del cron job (no_agent=True).

- Bit 0 di get_throttled = under-voltage NOW
- Cooldown: al massimo 1 alert ogni COOLDOWN_MIN se il problema persiste;
  un nuovo episodio (dopo un ritorno alla normalità) riavvisa subito.
"""
import json
import os
import re
