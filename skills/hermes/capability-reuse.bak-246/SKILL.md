...
### Manifest version-authority trap (v2.4.16, 2026-08-13)

Version strings can ALL agree (SKILL.md = plugin.yaml = protocol.py = 2.4.16)
while `evidence/deployment-manifest.json` still declares an older artifact
(2.4.7 selected/pending). **The deployment manifest — not the version
strings — is the authority for what is deployed.** A locally-ahead version is
successor source/WIP until: archive rebuild + sidecar SHA + internal surface
alignment + manifest update + clean deploy + live organic-hook proof.
Full T1-T6 validation battery (artifact identity, clean cohort, disposition
accounting, eligibility fail-closed, reversed/cross-peer, scope):
`references/v2.4.16-version-authority-and-test-battery-2026-08-13.md`.
...
### Analyzer cohort-filter hardcode trap (2026-08-13, fix in v2.4.6)

`batch-reuse-analyzer.py` (BOTH the runtime copy `~/.hermes/scripts/` and the skill copy) hardcoded the clean-cohort filter to `cohort_label=="v2.4.4_clean_live"` and `plugin_version=="2.4.4"`. After the v2.4.5/v2.4.6 releases every event carried the current cohort (`v2.4.5_review_queue`, plugin `2.4.6`), so the analyzer reported `legacy_or_pre_events == total_events` (1629/1629) and `clean == 0` — a FALSE "everything is legacy" report.

**Symptom:** report says legacy == total and clean == 0, but a direct check of events.jsonl shows all events already have the CURRENT `deployment_id`/`cohort_label`/`plugin_version`. The label is an analyzer filter artifact, NOT real legacy data. Verify before trusting the label:

```bash
python3 -c "
import json, collections
c = collections.Counter()
for l in open('$HOME/.hermes/data/reuse-observer/events.jsonl'):
    d = json.loads(l).get('data', {})
    c[(d.get('deployment_id'), d.get('cohort_label'), d.get('plugin_version'))] += 1
print(dict(c))"
```

**Fix:** read the cohort boundary from `cohort.json` dynamically instead of hardcoding:
...
clean = [r for r in rows if r[1].get("cohort_label")==cohort_label
         and r[1].get("plugin_version")==cohort_ver
         and r[1].get("deployment_id")==current_dep]
```

**Also align cohort.json itself:** it declared `plugin_version: 2.4.5` while events were actually emitted by plugin 2.4.6 — a stale manifest makes even the dynamic filter match nothing. Update `plugin_version` in cohort.json to the version events really carry (counter output above). Keep summary key NAMES unchanged (`v2.4.4_clean_live_events` etc.) — the dashboard reads latest.json but not those specific keys; renaming risks breaking consumers.

**Verification:** rerun analyzer → expect `clean == total`, `legacy == 0`, retrievals > 0.

**Chain-error noise in shadow mode (expected, not a bug):** `chain_correlation.errors` counts retrievals without `execute_code_started` (shadow retrievals never execute) plus raw execute_code without retrieval. Example: 306 errors / 98 retrievals = 98×3 (identifier_mismatch + start_count=0 + completion_count=0) + 12 raw execute_code. The chain check is designed for ACTIVE mode; in shadow it flags by-construction non-chains.

Full cleanup + fix detail: `references/analyzer-cohort-filter-and-cleanup-2026-08-13.md`.
