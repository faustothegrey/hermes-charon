#!/usr/bin/env python3
"""peer-health.py — HTTP API-based peer health monitor.

Alternative to peer-monitor.py (ping-based). Uses Hermes /health
endpoint on port 8642 instead of ICMP ping. Lighter, no subprocess
calls, and works in cron pre-run mode without triggering Tirith
scanning (no subprocess.run, no ping binary).

Key differences from peer-monitor.py:
  - No subprocess calls — pure urllib + json
  - No ARP discovery — only checks known peers
  - No system metrics (CPU temp, load, memory)
  - Extracts version info from /health JSON response
  - Only writes status.json + history.log (no STATUS.md)
  - Designed for cron pre-run script (script field in jobs.json)

To adapt for your network:
  1. Edit the PEERS dict at the top
  2. Run manually to test: python3 peer-health.py
  3. Register in cron as script field: 'peer-health.py'
"""
import json
import os
import sys
