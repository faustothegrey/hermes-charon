#!/usr/bin/env python3
"""
fritzbox-portmgr.py — AVM FritzBox port forwarding manager via TR-064.

Uses the fritzconnection library (pip install fritzconnection) to talk to the
FritzBox via its local TR-064 SOAP API. Supports listing, adding, and deleting
port forwarding rules without needing the web UI.

Usage:
  python3 fritzbox-portmgr.py info                      # router info
  python3 fritzbox-portmgr.py list                      # list all rules
  python3 fritzbox-portmgr.py add <port> <IP> [TCP|UDP] [desc]
  python3 fritzbox-portmgr.py del <port> [TCP|UDP]

Default: port ext=port int, protocol=TCP, desc="Hermes:<port>"

Environment variables:
  FRITZ_IP       (default: 192.168.178.1)
  FRITZ_USER     (default: fausto)
  FRITZ_PASSWORD (required for write operations — add/del)
"""
import sys
import os
...
def cmd_add(port, internal_ip, protocol="TCP", description=None):
    ...
    if not FRITZ_PASSWORD:
        print("❌ FRITZ_PASSWORD required for write operations")
        return
