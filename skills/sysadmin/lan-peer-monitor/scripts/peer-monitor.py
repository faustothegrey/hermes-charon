...
KNOWN_PEERS = [
    ...
    ("peer128", "Faustos-MacBook-Pro-Home-3.fritz.box", "Mac (macOS 26.5.1)"),
]
...
def discover_arp():
    ...
    for line in out.splitlines():
        ...
        state = parts[-1]
        mac = parts[3]
        if mac == "<incomplete>" or state == "FAILED":
            continue
...
def main():
    ...
    for name, host, desc in KNOWN_PEERS:
        ...
        if name == "peer70":
            md_lines.append(f"| **{name}** 🏆 | {host} | {desc} | {ico} ONLINE (orchestratore) |")
        else:
